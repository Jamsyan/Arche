...
class TestTrainingOrchestrator:
    async def test_process_job_dispatch(self):
        ...
        job = cast(
            TrainingJob,
            SimpleNamespace(id=uuid.uuid4(), orchestrator_step=Step.CREATING_INSTANCE),
        )
    ...
    async def test_process_idle_advances(self):
        ...
        job = cast(
            TrainingJob,
            SimpleNamespace(id=uuid.uuid4(), orchestrator_step=Step.IDLE),
        )
        await orch._process_job(job)
    ...
    async def test_step_create_instance_failure(self):
        ...
        job = cast(
            TrainingJob,
            SimpleNamespace(id=uuid.uuid4(), name="job", model_config={}),
        )
        await orch._step_create_instance(job)
    ...
    async def test_step_connect_ssh_retry_when_no_host(self):
        ...
        def _orch_container_get(name):
            if name == "db":
                return {"session_factory": _session_factory}
            return MagicMock()

        container = MagicMock()
        container.get = _orch_container_get
        orch = TrainingOrchestrator(container)
        orch._begin_step = AsyncMock()
        orch._retry_step = AsyncMock()
        job = cast(TrainingJob, SimpleNamespace(id=uuid.uuid4()))
    ...
    async def test_retry_step_fail_when_exhausted(self):
        ...
        def _orch_container_get_retry(name):
            if name == "db":
                return {"session_factory": _session_factory}
            return MagicMock()

        container = MagicMock()
        container.get = _orch_container_get_retry
        orch = TrainingOrchestrator(container)
        orch.MAX_RETRY = 1
        orch._fail_job = AsyncMock()
        job = cast(TrainingJob, SimpleNamespace(id=uuid.uuid4()))
