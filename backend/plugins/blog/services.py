...
class BlogService:
    ...
    async def get_post_detail_by_id(self, post_id: uuid.UUID) -> dict:
        """根据 ID 获取帖子详情（含标签，用于编辑器加载）。"""
    ...
    async def create_post(
        ...
        async with self.session_factory() as session:
            ...
            result = self._post_to_dict(post)
            ...
            if tags:
                for tag_name in tags[:MAX_TAGS_PER_POST]:
                    ...
                    async with self.session_factory() as s2:
                        s2.add(
                            BlogPostTag(post_id=post.id, tag_id=uuid.UUID(tag["id"]))
                        )
                    await s2.commit()
